...
class Application():
    ...
    def takeBoundedScreenShot(self, x1, y1, x2, y2):
        ...
        print(type(im))
        # img = cv2.imread(im)
        # cv2.imshow("First ScreenShot",img)
        # cv2.waitkey(2000)
        # cv2.destroyAllMethods()
        x = datetime.datetime.now()
        print(x)
        fileName = x.strftime("%f")
        print(fileName)
        im.save("C://Users//degal//Desktop/Snip/" + fileName + ".png")
        img=Image.open("C://Users//degal//Desktop/Snip/"+fileName+".png")
        txt=pytesseract.image_to_string(img)

        print(txt)
        print(type(txt))
        #currentdi=os.getcwd()
        #path=currentdi+"\Ashok"
        #os.mkdir(path,777)

        im.save("C://Users//degal//Desktop/Snip/" + fileName + ".png")
        # im.save(path+"/"+fileName+".png")
        # #path=path+'\SnipData.txt'
        # os.mkdir(path,755)
        fi = open("C://Users//degal//Desktop//SnipData.txt", 'a')
        # fi=open(path,"a")
        fi.write('\n')
        fi.write(txt)
        # print("ASHokjknjbdjsbdhsbbk")
        fi.close()
